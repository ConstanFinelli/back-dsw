...
import { findAll, add, findOne, update, deleteUser, hasBusiness, UserSchema, UserRegisterSchema, UserUpdateSchema, register} from "./user.controller.js";
...
const allowedFields = ['name', 'surname', 'email', 'password', 'category', 'categoryId', 'phoneNumber'];
const allowedFieldsRegister = ['name', 'surname', 'email', 'password', 'phoneNumber'];
...
userRouter.post("/register", validateSchemaWithParams(UserRegisterSchema, allowedFieldsRegister), register);
userRouter.get("/findOne/:id", authenticateWithCategories([Roles.ADMIN, Roles.OWNER]), findOne);
userRouter.put("/update/:id", authenticateWithCategories([Roles.ADMIN, Roles.OWNER]), validateSchemaWithParams(UserUpdateSchema, allowedFields), update);
