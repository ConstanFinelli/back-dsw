...
businessRouter.put("/update/:id", authenticateWithCategories([Roles.ADMIN, Roles.OWNER]),validateSchemaWithParams(BusinessSchema, allowedFields), update);
businessRouter.put("/activate/:id", activate);
businessRouter.delete("/remove/:id", authenticateWithCategories([Roles.ADMIN, Roles.OWNER]), remove);
